...
def get_nb_paths(
    ...
    for folder_path in folder_paths:
        try:
            # Check if the path is a file/notebook or a directory
            try:
                status = wobj.workspace.get_status(folder_path)
                
                # If it's a notebook or Python file, add it directly
                if status.object_type.name == "NOTEBOOK":
                    results.append((
                        status.path,
                        status.object_id,
                        "NOTEBOOK",
                    ...
                    logger.info(f"[workspace] Added notebook: {status.path}")
                elif status.object_type.name == "FILE" and status.path.endswith(".py"):
                    results.append((
                        status.path,
                        status.object_id,
                        "FILE",
                    ...
                    logger.info(f"[workspace] Added Python file: {status.path}")
                elif status.object_type.name == "DIRECTORY":
                    # It's a directory, recurse into it
                    _recursive_list(wobj, folder_path, exclusion_list, results)
                else:
                    logger.debug(f"Skipping unsupported object type: {status.object_type.name} at {folder_path}")
            except Exception as e:
                # If get_status fails, try listing as directory anyway
                logger.debug(f"Could not get status for {folder_path}, trying to list as directory: {e}")
                _recursive_list(wobj, folder_path, exclusion_list, results)
        except Exception as e:
