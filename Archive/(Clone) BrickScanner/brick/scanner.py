...
def run(
    ...
    try:
        ...
        if dry_run:
            ...
            _ = result.count()  # Trigger evaluation
            print("[scanner] Dry run complete.")
            return result
...
def ignore():
    ...
    # # ---------------------------------------------------------------------
    # # LLM processing - raw string -> Pydantic ReviewResponse
    # # ---------------------------------------------------------------------
    ...
    #     Uses a regex pre-pass to isolate the structured block from any
    #     surrounding noise, then delegates to Pydantic for validation.
    #     """
    ...
    # ) -> tuple[list[dict], list[tuple]]:
    #     """
    #     Run the review chain over each chunk, returning validated responses.
    ...
    # # ---------------------------------------------------------------------
    # # DataFrame construction - model_dump() -> pandas -> Spark
    # # ---------------------------------------------------------------------
    ...
    #     # Remaining part continues in Part 3
    #     if not rows:
    #         return spark.createDataFrame(
    #             pd.DataFrame(columns=_DF_COLUMNS)
    #         )
    ...
    #     df: DataFrame,
    # ) -> None:
    #     """Append scan findings to `genai_output`, filtering N/A severities."""
    ...
    #         SELECT
    #             t.notebook_id,
    #             element_at(split(t.notebook_path, '/'), -1) AS notebook_name,
    #             t.notebook_path,
    #             t.notebook_url,
    #             g.rule_name,
    #             t.comment,
    ...
    #             'Unresolved' AS review_comments,
    #             current_user() AS created_by,
    #             CAST(NULL AS TIMESTAMP) AS updated_by,
    #             current_timestamp() AS created_ts,
    #             current_timestamp() AS last_update_ts
    ...
    #     print("[scanner] Results saved to genai_output.")
    ...
    # ) -> Optional[DataFrame]:
    #     """
    #     Execute a BrickScanner code-quality scan.
    ...
    #         extractor_type:
    #             Label stored on results (e.g. "genai").

    #         include_paths:
    #             Comma-separated workspace paths to scan.

    #         dry_run:
    #             If True, return findings as a DataFrame without
    #             writing to the output table.

    #     Returns:
    #         DataFrame of non-N/A findings when dry_run=True,
    #         otherwise None.
    #     """
    ...
    #         if not rules:
    #             print("[scanner] No enabled rules -- nothing to do.")
    #             return None
    ...
    #         if not chunks:
    #             print("[scanner] No notebook content to process.")
    #             return None
    ...
    #         if dry_run:
    #             result = df.filter("severity != 'N/A'")
    #             result.count()  # trigger lazy evaluation
    #             return result
